...
<div class="d-flex align-items-center" style="max-height: 50px">
  ...
  <div class="input-group mx-auto join-group">
    <input
      ...
      v-model="joinCode"
    />
    <button class="btn btn-join" @click="joinDocument">
      Join
    </button>
  </div>
...
const joinCode = ref("");
const originalDocuments = ref([]); // Dữ liệu gốc của documents

const handleSearch = () => {
  if (!searchKeyword.value.trim()) {
    // Nếu từ khóa rỗng, trả lại danh sách tài liệu gốc
    documents.value = [...originalDocuments.value];
    return;
  }

  // Lọc tài liệu dựa trên tiêu đề
  documents.value = originalDocuments.value.filter((doc) =>
    doc.documentTitle
      .toLowerCase()
...
const fetchDocuments = async () => {
  ...
  if (result.data.documents) {
    ...
    originalDocuments.value = [...result.data.documents]; // Lưu danh sách gốc
  } else {
